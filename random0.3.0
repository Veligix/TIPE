...
#BUG DISQUES QUI VONT VERS LE HAUT/BAS -> DEPEND DU SENS DE RECHERCHE DANS LA SURFACE
#VOIR LOG2
...
<=> Conserver la distance au barycentre minimale n'est pas toujours souhaité
...
ex:
    si le barycentre fait -50x et +20y                             Ne semble pas nécessairement
    et qu'on veut bouger le disque en coordonnées x,y=a,b          être utile, y réfléchir
...
"""
Possibilité de déplacer un disque d'une certaine partie DANS le mur pour permettre à d'autres disqus d'autres déplacements qu'ils n'auraient pas pu faire s'il ne s'était pas déplacé dans le mur
De même, permettre à certains disques de se chevaucher d'une petite distance
pour permettre plus de déplacements pour débloquer un amas

Modifier la fct_cout comme une combinaison linéaire des scalaires à prendre en compte
et choisir des coefficients pour que ça marche au mieux

Tests: bloquer la sortie et voir si les disques s'étalent devant la sortie
Mettre un cul de sac pour bloquer volontairement les disques à un endroit donné
placer un déflecteur et observer la réaction

Restreindre le nombre de possibilités de déplacements par disque d'un amas
et augmenter le nombre de générations et de combinaisons calculées par amas

DANS UN AMAS PROCHE DE LA SORTIE
SI UN DISQUE SORT, ACTUELLEMENT, LE RESTE DE L'AMAS CONSIDERE QU'IL N'EST PAS ENCORE SORTI
ON DEPLACE UN DISQUE, IL SORT, ET LE RESTE DE L'AMAS VA SE DEPLACER A L'OPPOSE COMME POUR EVITER LE DISQUE QUI VIENT DE SORTIR, DONC QUI N'EXISTE PLUS

pour y remédier, dans chauqe combinaison d'amas calculée, si dans la combinaison actuellement
calculée, un disque sort, alors il faut mettre ses coordonnées en -200 -200 -200 -200
ou un truc comme ça (rajouter un if dans le append ligne 515 (environ) et si le disque
sort, il faut le mettre en append -200, et pas en append non sorti,
sinon il bloque les autres.

redéfinir la zone de recherche pour que la zone soit dirigée vers la sortie
et un peu en arrière de chaque disque (chaque disque peut reculer un peu par rapport à la sortie)


les calculs de gros amas (22 disques) sont TRES lents, voir si on peut optimiser
en placant moins de coordonnes à tester dans la zone possible


peut être calculer autant de familles que le nombre d'éléments dans l'amas
ou en tout cas en faire dépendre

sinon, un amas de taille 20, va tester par exemple 5 générations
donc en commençant le calcul par au maxiumum 5 disques distincts,
donc qui vont fortement influancer sur le mouvement global
donc les 15 autres disques, qui auraient pu créer un mouvement pertinent, ne vont pas être pris
en compte, et le déplacement sera extrêmement restreint
"""

"""
prioritaire:

    - redéfinir la zone de recherche (déplacer (?et restreindre))
    - modifier la fonction de cout avec la distance moyenne des disques au barycentre
    - faire dépendre le nombre d'itérations du nombre d'éléments dans l'amas considéré
        (trouver une petite égalité qui rendent peut être un peu plus optimisé)
"""
...
one=canvas.create_line(100,200,1100,200,width=5)    
two=canvas.create_line(1100,200,1100,700,width=5)    
three=canvas.create_line(1100,700,100,700,width=5)    
four=canvas.create_line(100,700,100,500,width=5)    
five=canvas.create_line(100,200,100,400,width=5) 
...
def fct_cout(f_n,f_n1,G):
    ...
    dist_S = sum([((i[0]-100)**2+(i[1]-450)**2)**1/2 for i in f_n1])  #Plus petit possible
    #d_Gn_Gn1 = (Gn_x-Gn1_x)**2+(Gn_y-Gn1_y)**2  #plus grand possible
    #print("f_n1:",f_n1)
    #print(dist_S)
    try:
        #return d_Gn_Gn1/(dist_S)
        return dist_S
...
class move(Thread):
    def run(self):
        global running,to_write,to_delete
        changed = True
        tour = 0
        l_amas = []
        while running:
                    ...
                    if len(i)>1:
                        for p in range(y): #Nombre d'étapes qu'on fait
                            ...
                            for s in new_fam:  #On prend les amas (s=une liste de coordonnées (tuples))
                                        ...
                                        for a in range(int(scale/5)):
                                            for j in range(int(scale/5)):
                                                if deplacement_possible(xa,ya,xo+5*a,yo+10*j,famille,d[2]):
                                                    ...
                                                    #create_mark(xo+a,yo+2*j,"red")
                                        taille = len(possibles)
                                        #sleep(0.01)
                                        #print(xa,ya,possibles)
                                        if taille!=0:
                                            chosen = randint(0,taille-1)
                                            """ch_x,ch_y = possibles[chosen][0],possibles[chosen][1]
                                            if ch_y<ya:
                                                count_up+=1
                                            elif ch_y>ya:
                                                count_down+=1"""
                                            #create_mark(ch_x,ch_y,"blue")
                                ...
                                """for j in new_fam:
                                    if j!=[]:
                                        notes.append((new_cout(s,j,(xi,yi)),j))"""
                        ...
                        b=new_cout(select)
                        if a<b:
                            #print("t_save:",to_save)
                            #print("wot:",new_fam)
                            #print(a,b)
                            #print("CALCULEE:",select,"\n\nNOT_MOVED:",not_moved[0])
                            for r in select:
                                xr,yr=r[0],r[1]
