...
for i in range(len(pos)):
    ...
    #Pour avoir des cibles en fonction du disque, on pourra crée une fonction
...
def deplacement_possible(x,y,famille_n):
    ...
    for i in voisinage:
        ...
        if sqrt((x-xb)**2+(y-yb)**2)<2*rayon:
            ...
            for j in famille_n:
                for s in j:
                    try:
                        if sqrt((s[0]-xb)**2+(s[1]-yb)**2)<2*rayon:
                            test=True
                    except IndexError:
                        pass
        if test:
            ...
            while len(l)!=0 and changed and tour<100000:
                ...
                for i in l_amas:
                    ...
                    try:
                        ...
                        pente = 0
                    epsilon = 2  #dÃ©placement en pixel  (1 n'est pas forcÃ©ment le min)
                    try:
                        assert abs(pente) != 1
                    ...
                    y=10 #nombre d'itérations de calcul de k familles
                    to_save = []
                    to_keep=[]
                    new_fam = [[(l[s][0],l[s][1]) for s in i]]
                    for p in range(y): #Nombre d'étapes qu'on fait
                        k = 30 #nombre de nouvelles corodonnées calculées pour chaque disque à chaque nouvelle famille pour chaque amas
                        famille=[[] for i in range(k+3)]
                        notes = []
                        for s in new_fam:
                            print("test")
                            for c in range(k):
                                for d in s:
                                    xa,ya =d[0],d[1]
                                    eps_x= randint(0,3)+1/randint(1,50) 
                                    eps_y= randint(0,3)+1/randint(1,50)
                                    x,y=xa+eps_x,ya+eps_y
                                    attempts = 0
                                    while not(deplacement_possible(x,y,famille)) and attempts<30:
                                        eps_x= randint(0,4)
                                        eps_y= randint(0,4)
                                        x,y=x+eps_x,y+eps_y
                                        attempts+=1
                                    if attempts == 30:
                                        famille[c].append((xa,ya))                                
                                    else:
                                        famille[c].append((x,y)) #On ajoute des nouvelles coordonnées
                            ...
                            for j in new_fam:
                                notes.append((fct_cout(s,j,(xi,yi)),j))
                            notes=sorted(notes,reverse=True) #Tri lexicographique, trie d'abord la fct_cout, ensuite l'indice i
                            for j in range(4):
                                to_save.append((notes[j][0],notes[j][1]))
                        to_save=sorted(to_save,reverse=True)
                        new_fam=[to_save[0][1],to_save[1][1],to_save[2][1],to_save[3][1]]
                    ...
                    for j in i:
                        ...
                        x,y = l[j][0],l[j][1]
                        index+=1
                        canvas.coords(l[j][7],x-rayon,
                                     y-rayon,x+rayon,y+rayon)
                        create_mark(x,y,"green")
                        print("MOVED")
                        ...
                        if x!=xo or y!=yo:
                            ...
                            print("stuck")
                            create_mark(x,y,"black")
                        if sqrt((x-xt)**2+(y-yt)**2)<5:   #on regarde si le disque
